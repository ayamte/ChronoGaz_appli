        ...
        <div className="relative flex flex-col w-full bg-white border-0 rounded-lg shadow-xl outline-none focus:outline-none max-h-[90vh] overflow-hidden">    
          ...
          <div className="relative p-6 flex-auto overflow-y-auto">    
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">    
              ...
              {/* ✅ NOUVEAU: Informations de Planification et Livraison */}
              {(order?.planification || order?.livraison) && (
                <div className="bg-green-50 p-4 rounded-lg shadow-sm lg:col-span-2 border-l-4 border-green-500">
                  <h3 className="flex items-center text-lg font-bold text-green-700 mb-3 border-b border-green-200 pb-2">
                    <Truck className="w-5 h-5 mr-2 text-green-600" />
                    Informations de Planification & Livraison
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Informations de planification */}
                    {order.planification && (
                      <div className="space-y-2">
                        <h4 className="text-sm font-semibold text-green-700 mb-2">📋 Planification</h4>
                        <div className="space-y-1">
                          <div className="flex justify-between">
                            <label className="text-xs font-medium text-green-600">État</label>
                            <span className="text-xs text-green-900 font-semibold">
                              {order.planification.etat || 'N/A'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <label className="text-xs font-medium text-green-600">Date de livraison</label>
                            <span className="text-xs text-green-900">
                              {order.planification.delivery_date ? 
                                new Date(order.planification.delivery_date).toLocaleDateString("fr-FR") : 'N/A'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <label className="text-xs font-medium text-green-600">Priorité</label>
                            <span className="text-xs text-green-900">
                              {order.planification.priority || 'N/A'}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    {order.livraison && (
                      <div className="space-y-2">
                        <h4 className="text-sm font-semibold text-green-700 mb-2">🚚 Livraison</h4>
                        <div className="space-y-1">
                          <div className="flex justify-between">
                            <label className="text-xs font-medium text-green-600">État</label>
                            <span className="text-xs text-green-900 font-semibold">
                              {order.livraison.etat || 'N/A'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <label className="text-xs font-medium text-green-600">Date de début</label>
                            <span className="text-xs text-green-900">
                              {order.livraison.date_debut_reel ? 
                                new Date(order.livraison.date_debut_reel).toLocaleDateString("fr-FR") : 'N/A'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <label className="text-xs font-medium text-green-600">Date de fin</label>
                            <span className="text-xs text-green-900">
                              {order.livraison.date_fin_reel ? 
                                new Date(order.livraison.date_fin_reel).toLocaleDateString("fr-FR") : 'N/A'}
                            </span>
                          </div>
                        </div>
                      </div>
